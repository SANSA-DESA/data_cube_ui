...
@task(name="slip.parse_parameters_from_task", base=BaseTask, bind=True)
...
@task(name="slip.validate_parameters", base=BaseTask, bind=True)
...
@task(name="slip.perform_task_chunking", base=BaseTask, bind=True)
...
@task(name="slip.start_chunk_processing", base=BaseTask, bind=True)
def start_chunk_processing(self, chunk_details, task_id=None):
    ...
    processing_pipeline = (group([
       ...
       | task_clean_up.si(task_id=task_id, task_model='SlipTask')).apply_async()

    return True


@task(name="slip.processing_task", acks_late=True, base=BaseTask, bind=True)
...
@task(name="slip.recombine_time_chunks", base=BaseTask, bind=True)
...
@task(name="slip.recombine_geographic_chunks", base=BaseTask, bind=True)
...
@task(name="slip.create_output_products", base=BaseTask, bind=True)
